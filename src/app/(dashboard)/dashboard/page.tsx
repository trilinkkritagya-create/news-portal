...
export default async function DashboardPage() {
  ...
  if (isMember) {
    ...
    return (
      <div className="flex flex-1 flex-col min-h-screen bg-[#F8FAFC] dark:bg-[#090e17] text-foreground">
        ...
        {/* Member Workspace */}
        <main className="flex-1 p-4 sm:p-6 lg:p-8 space-y-4 sm:space-y-6 max-w-[1550px] w-full mx-auto flex flex-col">
          <MemberDashboardView data={memberData} />
        </main>

        {/* Member Footer */}
        <footer className="mt-auto border-t border-[#E2E8F0] dark:border-slate-800 bg-card px-4 py-3 sm:px-6 flex flex-col sm:flex-row items-center justify-between font-mono text-[11px] text-muted-foreground gap-2 text-center sm:text-left select-none">
  ...
  return (
    <div className="flex flex-1 flex-col min-h-screen bg-[#F8FAFC] dark:bg-[#090e17] text-foreground">
      ...
      <main className="flex-1 p-4 sm:p-6 lg:p-8 space-y-4 sm:space-y-6 max-w-[1550px] w-full mx-auto flex flex-col">
        {/* Metric Cards: 2x2 grid on mobile/tablet, 4-col row on desktop */}
        <MetricCards stats={data.stats} />

        {/* Operational 3+1 Column Grid: Extended to fill available height cleanly */}
        <div className="grid grid-cols-1 xl:grid-cols-4 gap-4 sm:gap-6 items-stretch flex-1">
          ...
          <div className="xl:col-span-1 space-y-4 sm:space-y-6 min-w-0 flex flex-col justify-between">
      ...
      <footer className="mt-auto h-auto sm:h-9 py-2.5 sm:py-0 border-t border-[#E2E8F0] dark:border-slate-800 bg-card px-4 sm:px-6 flex flex-col sm:flex-row items-center justify-between text-[11px] text-muted-foreground font-mono select-none gap-2">
        <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2">
