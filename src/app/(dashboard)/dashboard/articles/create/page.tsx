...
export default function CreateArticlePage() {
  ...
  const [activeTab, setActiveTab] = useState<"draft" | "seo" | "preview">("draft");
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "An error occurred while saving article.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="flex flex-1 flex-col min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 pb-16 sm:pb-6">
        ...
        <div className="hidden sm:flex h-14 px-4 sm:px-6 md:px-8 items-center justify-between w-full">
          ...
          <div className="flex items-center space-x-3 sm:space-x-4">
            ...
            <div className="flex items-center gap-2.5">
              <span className="text-sm font-semibold text-slate-800 dark:text-slate-100 tracking-tight">Article Editor</span>
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        <main className="flex-1 overflow-y-auto px-4 sm:px-8 md:px-12 lg:px-16 py-8 space-y-7" data-purpose="article-editor">
        ...
        {showInspector && (
          ...
          >
            ...
            <div className="space-y-3">
              <span className="text-[11px] font-semibold uppercase tracking-wider text-slate-400">Content Metrics</span>
              <div className="grid grid-cols-2 gap-2">
                <div className="p-3 bg-slate-50 dark:bg-slate-800/60 rounded-lg border border-slate-100 dark:border-slate-700">
                  <span className="text-lg font-bold text-slate-900 dark:text-slate-100">{editorStats.words}</span>
                  <span className="block text-[10px] text-slate-400 mt-0.5">Total Words</span>
                </div>
                <div className="p-3 bg-slate-50 dark:bg-slate-800/60 rounded-lg border border-slate-100 dark:border-slate-700">
                  <span className="text-lg font-bold text-slate-900 dark:text-slate-100">{editorStats.chars}</span>
                  <span className="block text-[10px] text-slate-400 mt-0.5">Characters</span>
                </div>
              </div>
              <div className="p-3 bg-slate-50 dark:bg-slate-800/60 rounded-lg border border-slate-100 dark:border-slate-700 flex items-center justify-between">
                <div>
                  <span className="text-sm font-semibold text-slate-800 dark:text-slate-200">~{editorStats.readTimeMinutes} min read</span>
                  <span className="block text-[10px] text-slate-400">Estimated reading time</span>
                </div>
            ...
            <div className="space-y-2.5">
              <span className="text-[11px] font-semibold uppercase tracking-wider text-slate-400">Publishing Checklist</span>
              <div className="space-y-2 text-xs">
                <div className="flex items-center gap-2">
                  <span className={`w-4 h-4 rounded-full flex items-center justify-center text-[10px] ${title.trim() ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-400"}`}>
                    {title.trim() ? "✓" : "○"}
                  </span>
                ...
                <div className="flex items-center gap-2">
                  <span className={`w-4 h-4 rounded-full flex items-center justify-center text-[10px] ${excerpt.trim() ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-400"}`}>
                    {excerpt.trim() ? "✓" : "○"}
                  </span>
                  <span className={excerpt.trim() ? "text-slate-700 dark:text-slate-300" : "text-slate-400"}>
                    Excerpt summary provided
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <span className={`w-4 h-4 rounded-full flex items-center justify-center text-[10px] ${editorStats.words >= 50 ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-400"}`}>
                    {editorStats.words >= 50 ? "✓" : "○"}
                  </span>
                  <span className={editorStats.words >= 50 ? "text-slate-700 dark:text-slate-300" : "text-slate-400"}>
      ...
      >
        ...
        <div className="flex items-center gap-2 text-xs text-slate-500">
          <span className="font-medium text-slate-700 dark:text-slate-300">{editorStats.words} words</span>
      ...
      {showInspector && (
        <div className="sm:hidden fixed inset-x-0 bottom-[49px] z-30 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 p-4 shadow-xl space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold uppercase tracking-wider text-slate-500">Article Inspector</span>
            <button
              type="button"
              onClick={() => setShowInspector(false)}
          ...
          <div className="grid grid-cols-3 gap-2 text-center text-xs">
            <div className="p-2 bg-slate-50 dark:bg-slate-800 rounded border border-slate-100 dark:border-slate-700">
              <span className="block font-bold text-slate-900 dark:text-slate-100">{editorStats.words}</span>
              <span className="text-[10px] text-slate-400">Total Words</span>
            </div>
            <div className="p-2 bg-slate-50 dark:bg-slate-800 rounded border border-slate-100 dark:border-slate-700">
              <span className="block font-bold text-slate-900 dark:text-slate-100">{editorStats.chars}</span>
              <span className="text-[10px] text-slate-400">Characters</span>
            </div>
            <div className="p-2 bg-slate-50 dark:bg-slate-800 rounded border border-slate-100 dark:border-slate-700">
              <span className="block font-bold text-slate-900 dark:text-slate-100">~{editorStats.readTimeMinutes}m</span>
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-6">
          <div className="bg-white dark:bg-slate-900 rounded-xl max-w-3xl w-full max-h-[90vh] flex flex-col shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
            <div className="px-4 sm:px-6 py-3 sm:py-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-slate-50 dark:bg-slate-900/50">
              <span className="text-xs font-bold uppercase tracking-wider text-slate-500">Article Preview</span>
              <button
                type="button"
            ...
            <div className="p-5 sm:p-8 overflow-y-auto space-y-5 sm:space-y-6">
              ...
              <div
                ...
                dangerouslySetInnerHTML={{
                  __html: contentHtml || "<p class='text-slate-400 italic'>No content drafted yet.</p>",
                }}
              />
            </div>
          </div>
